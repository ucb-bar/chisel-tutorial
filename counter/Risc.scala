...
class RiscTests(c: Risc) extends CounterTester(c) {  
...
class RiscWrapper extends CounterWrapper(new CounterConfiguration) {
  val top = Module(new Risc)
...
class RiscWrapperTests(c: RiscWrapper) extends CounterWrapperTester(c) {
  def wr(addr: UInt, data: UInt)  = {
